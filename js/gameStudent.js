...
function qurAssigns(nth){
	nth =  (typeof(nth)=== 'number')? nth.toString() : nth ;
	q.equalTo("nth",nth);
	return q.find() ;
}
...
function getAssignByUser (assigns,userID){
	return assigns[assigns.getIndexByAttr('maker',userID)];
}

// put assigns inside
var assigns = [];
var userStatuses = [] ;
qurAssigns(srh.nth).then(function (a){
	assigns = a ;
	checkReadyAndGo();
},Log) ;
qurAllUserStatus().then(function(u){
	userStatuses = u ;
	checkReadyAndGo();
}.Log);

function checkReadyAndGo (){
	if (assigns.length !== 0 && userStatuses.length !== 0){
		formatToTable(assigns,userStatuses);
	}
}

function formatToTable(s,assigns){ // userstatus
	// generate a formatted array ;
	var arr = [] ;
	for (var i = 0 ; i < s.length ; i++){
		...
		arr2[0] = uu.get("photo");
		arr2[2] = ID;
		arr2[1] = uu.get("name")+"<span class='assignID'>"+getAssignByUser(assigns,uu.id)+"</span>"
		arr2[3] = ss.get("Level");
		arr2[4] = ss.get("XP");
		arr2[5] = ss.get("HP");		
		arr2[6] = ss.get("Life");				
